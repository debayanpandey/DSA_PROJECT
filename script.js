...
function playTone(freq, type, duration, volMultiplier = 1) {
    ...
    // Boost volume by ~127.5% on mobile devices to preserve previous 0.91 effective mobile gain
    const mobileBoost = window.innerWidth <= 768 ? 2.275 : 1.0;
    ...
    gainNode.gain.linearRampToValueAtTime(volume * volMultiplier * mobileBoost, audioCtx.currentTime + 0.01);
...
function playStopSound() {
    ...
    // Boost volume on mobile devices to preserve previous effective mobile gain
    const mobileBoost = window.innerWidth <= 768 ? 2.275 : 1.0;
    ...
    gainNode.gain.linearRampToValueAtTime(volume * 0.5 * mobileBoost, now + 0.03); // smooth attack, slightly boosted for low freq
