...
function updateSoundUI() {
    ...
    if (soundEnabled) {
        ...
        soundToggleBtn.style.color = 'var(--purple)';
        soundToggleBtn.style.borderColor = 'var(--purple)';
        soundToggleBtn.style.boxShadow = '0 0 8px rgba(184, 76, 255, 0.3)';
    } else {
        if (svgIcon) {
...
function updateSoundPosition() {
    if (!soundToggleBtn) return;
    const container = document.getElementById('sound-area');

    if (soundToggleBtn.parentElement !== container && container) {
        container.appendChild(soundToggleBtn);
    }
}
...
function handleArrayAction() {
    ...
    if (!isSame) {
        ...
        if (sizeSlider.tagName === 'SELECT') {
            let exists = Array.from(sizeSlider.options).some(opt => opt.value == arraySize);
            if (!exists) {
                const opt = new Option(`${arraySize} elements`, arraySize);
                sizeSlider.add(opt);
            }
        }
